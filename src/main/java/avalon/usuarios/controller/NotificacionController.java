...
public class NotificacionController {
    ...
    public ResponseEntity<Notificacion> updateNotificacion(@PathVariable Long notificacionId,
            @RequestBody UpdateNotificacionRequest request) {
        Notificacion notificacion = service.getNotificacion(notificacionId);

        if (notificacion != null) {
            Notificacion notificacionUpdate = service.updateNotificacion(notificacion, request);
            return notificacionUpdate != null ? ResponseEntity.ok(notificacionUpdate)
                    : ResponseEntity.badRequest().build();
        } else {
            return ResponseEntity.notFound().build();
        }
